  ...
  methods: {
    submit() {
      this.$axios({
        method: "post",
        url: "/comment",
        data: JSON.stringify({"question_id": this.$route.query.post_id,
          "parent_id":this.$route.query.author_id,
          "content": this.content
        })
      })
          .then(response => {
            if (response.code == 1000) {
              this.$router.push({path: this.redirect || "/post/"+this.$route.query.post_id});
            } else {
