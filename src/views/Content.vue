    ...
    <div class="left">
      <div class="container">
        ...
        <div class="l-container">
          ...
          <div class="user-btn">
            <span class="btn-item">
              <i class="iconfont icon-comment" @click="goComment"></i>comment(点击发表评论)
            </span>
          </div>
          <div>
            <h5>comment_list(评论列表)</h5>
            <ul >
              <li v-for="item in commentList" :key="item.message">
                用户{{item.Username }}  对文章评论为: {{item.content }}
              </li>
            </ul>
          </div>
        </div>
...
export default {
  ...
  methods:{
    goComment(){
      this.$router.push({ name: "Comment" ,query: {post_id:this.post.post_id,author_id:this.post.author_id}});
    },
    getPostDetail() {
      this.$axios({
        method: "get",
